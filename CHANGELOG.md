...
### Added
- README 改版：首屏是浅色、深色两张月度邮件截图（演示数据），加上 CI、Docker、Python、License 徽章，Mermaid 原理图，"邮件里有什么"图文说明和隐私说明；"本地试用"加上 `preview-email`。
- `SECURITY.md`（英文）：支持的版本、通过 GitHub 私下报告漏洞的方式、安全设计要点。
- 账单月邮件的分类和平时比：明显偏离平时的分类，名字下面加一行灰色小字"比平时多 ¥800"或"比平时少 ¥400"。平时指前 3 个月的中位数；差额至少 ¥300 且至少 30% 才标，最多 3 个；"其他"和"未分类"不比。纯文本版本也有。
- 账单月邮件新增"近 6 个月"：每个账单月一根柱子（Apple Card 月度活动的样式），本月用强调色、之前的月份用灰色，下面一行给出平均值和本月比平均多或少多少；口径和"本月消费"一致，没有账单的月份写"—"，有账单的月份不到 2 个时不显示。纯文本版本也有这一行。
- 每月备份：账单月那封邮件附一份压缩的数据库（490 KB 压成约 120 KB），服务器上也留最近 6 份在 `data/backups/`。用 SQLite 自己的备份接口，WAL 里还没落盘的改动也备得到；`resend` 重发不带附件。配置 `backup: {enabled, keep}`，还原步骤见 docs/deploy.md#从备份还原。
...
### Changed
- README 改为英文为主（`README.md`），简体中文版改名为 `README.zh-CN.md`；License 和 Security 相关内容只用英文。
- 账单月邮件去掉"每日消费"柱状图和"最大的一笔"：各张卡的账单周期错开，合起来画的每日图两头只算到部分卡，日均也被拉低；最大的一笔经常和商户排行第一名重复。标准账单（`autobill statement`）里的每日图保留。

### Fixed
- 解析时的任何错误（中行 PDF 损坏或加密、数值不符合数据模型等）都记为 `FAILED` 并发提醒；以前只接住 `TemplateChanged` 和格式错误，其他错误会让 `run` / `serve` 每次都停在同一封邮件上，后面的账单全被挡住，也没有提醒。
- 数据库备份失败（比如 `backups/` 没有写权限）不再挡住月度邮件：邮件照常发出，日志里打印一行警告。
- 月度邮件圆盘图里的商户名做了转义，名字里带引号、`<`、`&` 不会再破坏邮件的 HTML。
