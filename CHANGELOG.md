...
### Added
- `scripts/export_ai_categories.py`：把 AI 分好的商户（`ai_categories`）导出成可以合并进 `rules.example.yaml` 的规则片段，进了 Git 就有备份，而且规则的优先级高于 AI 答案。**只读**打开数据库（`mode=ro`），`serve` 在跑也能用。解析器认不出商户时 `ai_categories.merchant` 存的是原始描述、可能带着别人的姓名，所以分三档：可公开的直接打印，两三个汉字的名字（可能是连锁店也可能是人名）要作者用 `--review` 自己判断，原始描述和带支付渠道字样的扣在服务器上（`--show-held` 才列）。只导出商户名、地点、币种和把握程度，不导出金额、日期、模型名和 AI 给的理由。
