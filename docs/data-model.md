...
- **M3 已建**：`emails`、`bills`、`bill_balances`、`transactions`、`fx_rates`。**M7c 加了** `cycle_threads`（表结构版本 2），**M8a 加了** `folder_cursors`（版本 3），**M8b 加了** `alerts`（版本 4）；旧数据库打开时自动升级。其余几张表（`folder_cursors`、`accounts`/`cards`、`runs`）到 M8 再建。
