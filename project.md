...
| 阶段 | 内容 | 验收 |
|:---|:---|:---|
| **第一版（当前）** | M0 骨架 → M1 模型和工具函数 → M2 农行解析器 → M3 第一条完整链路 → M4 建行 → M5 中行（`v0.1.0`）→ M6 分类和报表 → M7 邮件报表 → **M7b 标准账单** → **M7c 账单月进度邮件** → **M7d 分类覆盖率和 AI 接口** → M8 IMAP 和定时运行（`v0.2.0`） | 每一步的标准见 [development.md §5](docs/development.md#5-里程碑第一版m0m8)；最终标准是一封真实账单从转发到收到报表全程自动完成 |
