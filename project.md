...
- **定位**：自用优先、轻量的开源数据管道，**公开仓库**。
  ...
  - **解析不用 AI**，全部用确定性规则；AI 只用来给分类提建议，接口已预留（见下方决策表）。
  - **只做汇总分析，并展示还款日**；不做还款提醒，也不追踪是否已还款。
- **手机端**：不做 App，在 iPhone 的苹果邮件里看报表（iCloud 邮箱），排版追求 iOS 原生 App 的质感。第一眼只看汇总和可视化，逐笔流水折叠在邮件里，完整账单是附件 PDF。
...
| 邮件来源 | QQ 主邮箱按银行发件人自动转发到**作者自己的 iCloud 邮箱的一个别名**，iCloud 规则把它移进 `AutoBill` 文件夹；程序用 **App 专用密码**只读这个文件夹（2026-09-19 定，不用再注册专门的中心邮箱）。这个 iCloud 邮箱只用于 AutoBill；App 专用密码能访问它的邮件、通讯录和日历，作者已确认接受。**不读 QQ 主邮箱**。历史账单分小批"作为附件"转发；只回填最近 12 个月。见 [docs/setup.md](docs/setup.md) |
...
| 部署 | 原定家里的 Windows 电脑 + 计划任务。**2026-09-19 作者倾向放到 Oracle 免费服务器**（QQ 主邮箱转发到中心邮箱，服务器定时拉取），M8 时定：代码写成 Windows 计划任务和 Linux cron/systemd 都能跑。服务器上要装 Chromium 和中文字体 `fonts-noto-cjk`（打印 PDF），时区按北京时间。第一版不用 Docker（列入"以后"） |
...
[银行账单邮件] ──自动转发──┐        ┌──批量"作为附件"转发── [历史账单]
 ...
 Store       SQLite（WAL）：一个事务里完成 upsert 账单和替换流水 · fx_rates 汇率缓存
                           ▼
 Report      账单月进度邮件 + 标准账单 PDF 附件 → iCloud 邮箱（发送成功记下 reported_at）
