...
from autobill.report.statement import StatementView, build_view, render_statement_html
from autobill.report.style import COLORS, Row, card_label, category_bar_rows, money
...
class CycleReport:
    ...
    def progress(self) -> str:
        return f"已出账 {self._count('arrived')}/{len(self.cards)}"

    @property
    def status_line(self) -> str:
        missing = self._count("missing")
        maybe = f"，{missing} 张可能无账单" if missing else ""
        if self.complete:
            return f"本月账单已齐{maybe}"
        return f"还有 {self._count('pending')} 张待出账{maybe}"
...
def _arrived_note(bill: Bill, view: StatementView) -> str:
    parts = [f"{bill.statement_date:%m-%d} 出账"]
    if not view.nothing_due:
        parts.append(f"{bill.due_date:%m-%d} 前还款" if bill.due_date else "还款日未知")
    if any(b.currency != "CNY" and b.amount_due for b in bill.balances):
        parts.append("外币已折合")
    if bill.status != "OK":
    ...
    for account in sorted(expected, key=lambda a: (expected[a] or 32, a)):
        ...
        if bill_id is None:
            if today > _usual_date(cycle, day) + GRACE:
                when = f"通常 {day} 号出账，已过一周" if day else "本月已过去"
                cards.append(CardState(label, "missing", f"{when}，本月可能没有账单"))
            else:
                when = f"通常 {day} 号左右出账" if day else "本月还没出账"
                cards.append(CardState(label, "pending", when))
            continue
        ...
        cards.append(
            CardState(label, "arrived", _arrived_note(bill, view), amount, bill_id in new_ids)
        )
        if bill_id in new_ids:
            new_bills.append(NewBill(label, view, attachments.get(bill_id, "")))
        ...
        if not view.nothing_due and bill.due_date:
            d = bill.due_date
            line = DueLine(f"{d.month}月{d.day}日", f"周{WEEKDAYS[d.weekday()]}", label, amount)
...
def render_cycle_html(report: CycleReport) -> str:
    return _env.get_template("cycle_report.html.j2").render(r=report, c=COLORS)
...
def plain_text(report: CycleReport) -> str:
    ...
    if report.complete and report.due_lines:
        out += ["", "还款日一览："]
        out += [f"{d.day} {d.weekday} {d.label} {d.amount}" for d in report.due_lines]
    return "\n".join(out)
